...
function showView(name) {
  ...
  document.getElementById('viewTitle').textContent = { overview: 'نظرة عامة', products: 'المنتجات', orders: 'الطلبات', settings: 'إعدادات المتجر', content: 'المحتوى و SEO', backup: 'نسخ البيانات' }[name] || 'نظرة عامة';
...
function fillForm(id) {
  ...
  document.getElementById('dialogTitle').textContent = item ? 'تعديل المنتج' : 'إضافة منتج'; form.reset();
...
function updateImagePreview(source) { const image = document.getElementById('imagePreview'); const text = document.getElementById('imagePreviewText'); if (source) { image.src = source; image.hidden = false; text.hidden = true; image.onerror = () => { image.hidden = true; text.hidden = false; text.textContent = 'تعذر عرض الصورة'; }; } else { image.hidden = true; text.hidden = false; text.textContent = 'لم يتم اختيار صورة بعد'; } }
...
function renderOrders() {
  ...
  if (!list.length) { target.innerHTML = '<div class="empty-panel"><div class="empty-icon">▤</div><h3>لا توجد طلبات حالياً</h3><p>عند وصول أول طلب من المتجر، سيظهر هنا بجميع تفاصيله.</p></div>'; return; }
  target.innerHTML = `<div class="orders-list">${list.map(order => `<article class="order-card"><div class="order-top"><div><strong>${esc(order.id)}</strong><small>${new Date(order.createdAt).toLocaleString('fr-MA')}</small></div><select data-order-status="${esc(order.id)}">${Object.entries(statuses).map(([key,label]) => `<option value="${key}" ${order.status === key ? 'selected' : ''}>${label}</option>`).join('')}</select></div><div class="order-customer"><strong>${esc(order.customer.name)}</strong><a href="tel:${esc(order.customer.phone)}">${esc(order.customer.phone)}</a><span>${esc(order.customer.address)}</span></div><div class="order-items">${order.items.map(item => { const product = state.products.find(entry => String(entry.id) === String(item.id)); return `<span>${esc(product?.name.ar || item.name || 'منتج')} × ${item.qty} <b>${esc(item.size || 'M')}</b></span>`; }).join('')}</div><div class="order-bottom"><span>الدفع عند الاستلام</span><strong>${money(order.total)}</strong></div></article>`).join('')}</div>`;
}
...
document.addEventListener('click', event => {
  ...
  if (action.dataset.action === 'delete' && confirm('هل أنت متأكد من حذف هذا المنتج؟')) { state.products = state.products.filter(item => String(item.id) !== String(action.dataset.id)); save('تم حذف المنتج'); renderAll(); }
  if (action.dataset.action === 'save-settings') { const data = dataFrom(document.getElementById('settingsForm')); state.settings = { ...state.settings, ...data, deliveryFee: Number(data.deliveryFee || 0), freeDeliveryFrom: Number(data.freeDeliveryFrom || 0), languagesAr: data.languagesAr === 'on', languagesFr: data.languagesFr === 'on', languagesEn: data.languagesEn === 'on' }; save('تم حفظ إعدادات المتجر'); syncSettingsRemote().catch(() => toast('تعذر حفظ الإعدادات في Supabase')); }
  if (action.dataset.action === 'save-content') { state.content = { ...state.content, ...dataFrom(document.getElementById('contentForm')) }; save('تم حفظ إعدادات المحتوى'); }
  if (action.dataset.action === 'export') exportData();
  if (action.dataset.action === 'reset' && confirm('هل أنت متأكد من استعادة البيانات الأصلية؟')) { state = { products: copy(seedProducts), settings: copy(defaults.settings), content: copy(defaults.content) }; save('تمت استعادة البيانات الأصلية'); renderAll(); }
});
document.getElementById('productForm').addEventListener('submit', async event => { event.preventDefault(); const form = event.target; const submit = document.getElementById('submitProduct'); const file = form.elements.imageFile.files[0]; submit.disabled = true; submit.textContent = file ? 'جاري رفع الصورة...' : 'جاري الحفظ...'; try { if (file && remoteUser && window.AlwatinDB) form.elements.image.value = await AlwatinDB.uploadImage(file); const product = productFromForm(form); const index = state.products.findIndex(item => String(item.id) === String(product.id)); if (index < 0) state.products.unshift(product); else state.products[index] = product; document.getElementById('productDialog').close(); save(index < 0 ? 'تمت إضافة المنتج' : 'تم تحديث المنتج'); renderAll(); await syncProductRemote(product); } catch { toast('تم الحفظ محلياً. تأكد من Storage ومخطط Supabase'); } finally { submit.disabled = false; submit.textContent = 'حفظ المنتج'; } });
